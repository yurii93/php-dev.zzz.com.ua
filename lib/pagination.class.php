...
class Pagination
{
    ...
    public function get()
    {
        ...
        if ($this->total > $this->limit) {
            ...
            $box = '';
            for($i=2; $i < $this->amount; $i++) {
                $box .= $this->generateHtml($i, null);
            }

            $box = '<span class="pagination" id="hidden-pagination" style="margin-bottom: -15px; display: none;">' . $box . '</span><br>';

            $html = $box . '<ul class="pagination">';

            if($this->amount > 2) {
                $points = '<li id="special-pagination"><span>...</span></li>';
            } else {
                $points = '';
            }

            $links = $this->generateHtml(1, null) . $points . $this->generateHtml($this->amount, null);
    ...
    private function generateHtml($page, $text = null, $atribute = null)
    {
        ...
        return
            '<li><a ' . "title='$atribute'" . ' href="' . $currentURI . $this->index . $page . '">' . $text . '</a></li>';
    }
